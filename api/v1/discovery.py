    ...
    try:
        ...
        for network_range in network_ranges:
            try:
                ...
                raise HTTPException(
                    status_code=400, 
                    detail=f"Invalid network range '{network_range}': {str(e)}"
                )
...
async def list_discovery_jobs(db = Depends(get_db)):
    """List discovery jobs"""
    logger.info("Discovery jobs list request")
    
    try:
        jobs = db.query(DiscoveryJobModel).order_by(DiscoveryJobModel.created_at.desc()).all()
        
        result = []
        for job in jobs:
            result.append(DiscoveryJob(
                id=job.id,
                name=job.name,
...
async def get_discovery_job(job_id: int, db = Depends(get_db)):
    """Get discovery job details"""
    logger.info(f"Discovery job details request: {job_id}")
    
    try:
        job = db.query(DiscoveryJobModel).filter(DiscoveryJobModel.id == job_id).first()
...
async def cancel_discovery_job(job_id: int, db = Depends(get_db)):
    ...
    try:
        ...
        if job_id in network_discovery_service.discovery_jobs:
            task = network_discovery_service.discovery_jobs[job_id]
            if not task.done():
                task.cancel()
                try:
                    await task
...
async def get_discovery_results(job_id: int, db = Depends(get_db)):
    ...
    try:
        ...
        discovered_devices = db.query(Device).filter(
            Device.created_at >= job_start_time,
            Device.created_at <= job_end_time
        ).all()
...
async def run_discovery_job(job_id: int, network_ranges: List[str], discovery_types: List[DiscoveryType]):
    ...
    try:
        ...
        result = await network_discovery_service.start_discovery_job(
            job_id, network_ranges, discovery_types
        )
        
        if result.success:
            logger.info(f"Discovery job {job_id} started successfully")
        else:
            logger.error(f"Discovery job {job_id} failed to start: {result.error_message}")
