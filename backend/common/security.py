...
def _get_cipher_suite():
    """Get cipher suite with robust key handling"""
    try:
        if isinstance(ENCRYPTION_KEY, bytes):
            return Fernet(ENCRYPTION_KEY)
        elif isinstance(ENCRYPTION_KEY, str):
            # Try to decode as base64, if that fails, use the string directly
            try:
                decoded_key = base64.urlsafe_b64decode(ENCRYPTION_KEY)
                return Fernet(decoded_key)
            except (ValueError, binascii.Error):
                # If base64 decoding fails, use the string as-is (pad if needed)
        ...
        logger.warning(f"Failed to create cipher suite: {e}, generating new key")
        return Fernet(Fernet.generate_key())
