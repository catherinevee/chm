...
class AlertRule(Base):
    ...
    # Scope and targeting
    device_ids = Column(JSON, nullable=True, index=True)  # Target devices
    metric_names = Column(JSON, nullable=True, index=True)  # Target metrics
    device_groups = Column(JSON, nullable=True)  # Device group tags
    user_groups = Column(JSON, nullable=True)   # User group tags
    ...
    notification_channels = Column(JSON, nullable=True)  # Default channels
    
    # Testing and validation
    test_mode = Column(Boolean, default=False, nullable=False)
    test_results = Column(JSON, nullable=True)  # Test execution results
    validation_errors = Column(JSON, nullable=True)  # Validation issues
    ...
    __table_args__ = (
        ...
        # Note: Removed JSON column indexes as they require special operators in PostgreSQL
        # Index('idx_alert_rules_devices', 'device_ids', 'status'),  # JSON column
        # Index('idx_alert_rules_metrics', 'metric_names', 'status'),  # JSON column
