...
class Settings(BaseSettings):
    ...
    session_ttl: int = Field(default=3600, env="SESSION_TTL")  # Session TTL in seconds (1 hour default)
    max_sessions_per_user: int = Field(default=5, env="MAX_SESSIONS_PER_USER")  # Maximum concurrent sessions
    max_devices_per_user: int = Field(default=10, env="MAX_DEVICES_PER_USER")  # Maximum devices per user
    ...
    mfa_issuer: str = Field(default="CHM", env="MFA_ISSUER")
    ...
    audit_log_sensitive_data: bool = Field(default=False, env="AUDIT_LOG_SENSITIVE_DATA")
    audit_log_to_file: bool = Field(default=False, env="AUDIT_LOG_TO_FILE")
    audit_log_file_path: Optional[str] = Field(default=None, env="AUDIT_LOG_FILE_PATH")
    audit_log_path: Optional[str] = Field(default="./logs/audit.log", env="AUDIT_LOG_PATH")
    compliance_mode: bool = Field(default=False, env="COMPLIANCE_MODE")
    encrypt_audit_data: bool = Field(default=False, env="ENCRYPT_AUDIT_DATA")
    hash_pii_in_audit: bool = Field(default=True, env="HASH_PII_IN_AUDIT")
    audit_include_response_body: bool = Field(default=False, env="AUDIT_INCLUDE_RESPONSE_BODY")
    audit_include_request_body: bool = Field(default=False, env="AUDIT_INCLUDE_REQUEST_BODY")
